function varargout = complex_plot(u, varargin)
...
set(gcf, 'Color', 'w');
%
if strcmp(p.Results.colorbar,'inside')
    colormap( squeeze(C) )
%     c = colorbar('Location','West');
%     set(c,'YTick',[])
    c = colorbar('Location','West','Ticks',[0.05,0.95],'TickLabels', {'\color{white} 0', '\color{white} 2\pi'});
%     c.TickLabels = {'0', '2\pi'} ;
elseif strcmp(p.Results.colorbar,'outside')
    colormap( squeeze(C) )
    c = colorbar('Location','WestOutside');
%     set(c,'YTick',[])
    c = colorbar('Location','WestOutside','Ticks',[0.05,0.95],'TickLabels', {'\color{white} 0', '\color{white} 2\pi'});
%     c.TickLabels = {'0', '2\pi'} ;
