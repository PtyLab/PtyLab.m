...
[R, C] = generate_fermat_spiral(200, 'scaling', 20);
...
[Ropt, Copt] = add_checkpointing(Ropt, Copt, number_of_checkpoints);
...
figure(2)
plot(Copt(1:end), Ropt(1:end), '-','Color',[0 0 0]); hold on
% for k = 1:length(Ropt)-1
% plot(Copt(k:k+1), Ropt(k:k+1), '-','Color',cmap(k,:)); hold on
% end
